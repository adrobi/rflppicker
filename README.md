...
Для проверки установки используйте собственный небольшой CSV/TSV-файл с
вариантами. Локальный `variant4.txt` является тестовым файлом разработчика и
игнорируется Git.
...
Перед публикацией в Git добавьте в коммит код, `README.md`, `requirements.txt`,
тесты и небольшие входные примеры. Большие FASTA и автоматически созданные
файлы из `results/` обычно удобнее хранить отдельно или через Git LFS.
...
git init
git add .
git commit -m "Initial RFLP Picker version"
git status
```

Для удалённого репозитория добавьте адрес `git remote add origin <URL>`, затем
выполните `git branch -M main` и `git push -u origin main`.
